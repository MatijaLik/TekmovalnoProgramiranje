...
void prop(Treap *me) {
    ...
    if(me->invert == true){
        for (Treap *t:me->kids) if (t != NULL) {
            if(t->flip or t->fill!=-1) prop(t);
            t->invert = not(t->invert);
        }
        me->data = 1-me->data;
        me->sum = size(me) - me->sum;
    ...
    if(me->flip == true){
        for (Treap *t:me->kids) if (t != NULL){
            if(t->invert or t->fill!=-1) prop(t);
            t->flip = not(t->flip);
        }
        swap(me->kids[0], me->kids[1]);
        swap(me->pref0, me->suf0);
        swap(me->pref1, me->suf1);
        me->flip=false;
    }
    if(me->fill != -1){
        for (Treap *t:me->kids) if (t != NULL) {
            if(t->invert or t->flip) prop(t);
            t->fill = me->fill;
        }
 
        me->data=me->fill;
        me->sum = (me->fill == 1) ? size(me) : 0;

        me->pref0 = (me->fill == 0) ? size(me) : 0;
        me->best0 = (me->fill == 0) ? size(me) : 0;
        me->suf0 = (me->fill == 0) ? size(me) : 0;
        
        me->pref1 = (me->fill == 1) ? size(me) : 0;
        me->best1 = (me->fill == 1) ? size(me) : 0;
        me->suf1 = (me->fill == 1) ? size(me) : 0;
